...
export default function App() {
  ...
  function getCardPower(card, currentTrump, leadSuit) {
    ...
    if (card.suit === currentTrump) {
      ...
      // --- CORREÇÃO DA RENGA ---
      // Ordem: A (maior) > 3 > 7 > 2 (menor das fortes)
      if (card.label === "A") power += 100; // Ás ganha de tudo
      if (card.label === "3") power += 80; // 3 ganha de 7 e 2
      if (card.label === "7") power += 60; // 7 ganha de 2
      if (card.label === "2") power += 40; // 2 ganha do resto (K, J, Q...)
    }
  ...
  const getCardPoints = (card, currentTrump) => {
    ...
    // 7 vale 10 (seja trunfo ou não, geralmente é a bisca)
    if (label === "7") return 10;

    // Cartas que valem 10 SÓ se forem TRUNFO (3 e 2)
    if (isTrump) {
      if (label === "3") return 10;
      if (label === "2") return 10;

      // Pontos menores do trunfo (se houver na sua regra)
      if (label === "4") return 4;
      if (label === "5") return 3;
      if (label === "6") return 2;
    }
